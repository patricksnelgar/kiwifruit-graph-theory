...
vine9_nodes <- full_join(vine9_sources, vine9_targets, by = "label") %>%
	arrange(label) %>%
	full_join(select(vine9_data, to, length, to_shoot_id, x, y, quadrant, cane_id, to_origin_id, base_origin_id, notes), by = c("label" = "to"))
...
ggsave("output/graphs/kiwimac_vine9_layout.png", width = 20, height = 20)


ggraph(vine9_graph, "tree") +
	geom_edge_link(colour = "brown") +
...
ggsave("output/graphs/kiwimac_vine9.png", width = 35, height = 20)
