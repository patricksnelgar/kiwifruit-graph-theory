...
vine2_basic_DryMatter <- ggplot(filter(vine2_links, !is.na(to))) +
	...
	geom_point(aes(x = yend, y = xend, colour = target_type), 
			   data = filter(vine2_links, !is.na(to) & target_type == "Shoot"),
			   shape = 19, 
			   size = 4) + 
	...
	geom_jitter(aes(yend, xend, fill = dm_bins), 
				data = filter(vine2_fruit_data, !is.na(dm_bins)), 
				alpha = 0.8, 
				size = 4,
				width = 10,
				height = 70,
	...
	guides(size = FALSE) +
	ggtitle("Kiwimac vine2 - basic Dry Matter heatmap") +
	labs(x = NULL, y = NULL) +
	theme_bw() + 
...
ggsave("./output/vine 2/basic Dry Matter heatmap.jpg", width = 16, height = 12)
...
ggsave("output/vine 2/x-and-y_layout.png", width = 20, height = 20)
...
ggsave("output/vine 2/tree_architecture.png", width = 35, height = 20)
