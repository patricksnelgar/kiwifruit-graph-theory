...
vine5_fruit_data <- read_csv("input/fruit_data_vine5.csv") %>%
	select(FruitID:Height, FrtWt:DM) %>%
	mutate(shoot_id = paste("5", ShootID, sep = "-"))
...
vine5_basic_dm <- ggplot(filter(vine5_links, !is.na(to))) +
	...
	guides(size = FALSE) +
	ggtitle("Kiwimac vine5 - basic DM heatmap") +
	labs(x = NULL, y = NULL) +
	theme_bw() + 
...
ggsave("./output/vine 5/basic DM heatmap.jpg", width = 16, height = 12)
