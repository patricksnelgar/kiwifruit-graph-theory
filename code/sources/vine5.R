...
vine5_data <- read_csv("input/kiwimac_data_vine5.csv") %>%
...
vine5_nodes <- full_join(vine5_sources, vine5_targets, by = "label") %>%
	arrange(label) %>%
	full_join(select(vine5_data, to, length, to_shoot_id, x, y, quadrant, cane_id, to_origin_id, base_origin_id, notes), by = c("label" = "to"))
...
ggsave("output/graphs/kiwimac_vine5_layout.png", width = 20, height = 20)


ggraph(vine5_graph, 'tree') +
	geom_edge_link(colour = "brown") +
...
ggsave("output/graphs/kiwimac_vine5.png", width = 35, height = 20)
