#### data import ####
vine8_data <- read_csv("input/architecture/kiwimac_data_vine8.csv") %>%
...
vine8_fruit_data <- read_csv("input/fruit_data/fruit_data_vine8.csv") %>%
...
vine8_nodes <- full_join(vine8_sources, vine8_targets, by = "label") %>%
	arrange(label) %>%
	full_join(select(vine8_data, to, length, to_shoot_id, x, y, quadrant, diameter, cane_id, to_origin_id, base_origin_id, notes), by = c("label" = "to"))
...
vine8_links <- vine8_data %>%
	select(from, to) %>%
	full_join(select(vine8_nodes, label, x_pos, y_pos), by = c("from" = "label")) %>%
	rename(xstart = x_pos, ystart = y_pos) %>%
	full_join(vine8_nodes, by = c("to" = "label")) %>%
	rename(xend = x_pos, yend = y_pos) 

vine8_fruit_data %<>%
	left_join(., select(vine8_links, to_shoot_id, xend, yend), by = c("shoot_id" = "to_shoot_id")) 
...
vine8_basic_DryMatter <- ggplot(filter(vine8_links, !is.na(to))) +
	...
	geom_jitter(aes(yend, xend, fill = dm_bins), 
				data = filter(vine8_fruit_data, !is.na(dm_bins)), 
				alpha = 0.8, 
				size = 4,
				width = 10,
				height = 70,
	...
	guides(size = FALSE) +
	ggtitle("Kiwimac vine8 - basic Dry Matter heatmap") +
	labs(x = NULL, y = NULL) +
	theme_bw() + 
...
ggsave("./output/vine 8/basic Dry Matter heatmap.jpg", width = 16, height = 12)
...
ggraph(vine8_graph, layout = "manual", x = vine8_nodes$y_pos, y = vine8_nodes$x_pos) +
