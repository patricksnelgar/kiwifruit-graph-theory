...
vine3_nodes <- full_join(vine3_sources, vine3_targets, by = "label") %>%
	arrange(label) %>%
	full_join(select(vine3_data, to, length, to_shoot_id, x, y, quadrant, cane_id, to_origin_id, base_origin_id, notes), by = c("label" = "to"))
...
ggraph(vine3_graph, layout = "manual", x = vine3_nodes$x_pos, y = vine3_nodes$y_pos) +
...
ggsave("output/graphs/kiwimac_vine3_layout.png", width = 20, height = 20)
