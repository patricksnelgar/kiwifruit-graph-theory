...
vine7_basic_DryMatter <- ggplot(filter(vine7_links, !is.na(to))) +
	...
	geom_jitter(aes(yend, xend, fill = dm_bins), 
				data = filter(vine7_fruit_data, !is.na(dm_bins)), 
				alpha = 0.8, 
				size = 4,
				width = 10,
				height = 70,
	...
	guides(size = FALSE) +
	ggtitle("Kiwimac vine7 - basic Dry Matter heatmap") +
	labs(x = NULL, y = NULL) +
	theme_bw() + 
...
ggsave("./output/vine 7/basic Dry Matter heatmap.jpg", width = 16, height = 12)
