#### data import ####
vine3_data <- read_csv("input/architecture/kiwimac_data_vine3.csv") %>%
...
vine3_fruit_data <- read_csv("input/fruit_data/fruit_data_vine3.csv") %>%
...
vine3_nodes <- full_join(vine3_sources, vine3_targets, by = "label") %>%
	arrange(label) %>%
	full_join(select(vine3_data, to, length, to_shoot_id, x, y, quadrant, diameter, cane_id, to_origin_id, base_origin_id, notes), by = c("label" = "to"))
...
vine3_links <- vine3_data %>%
	select(from, to) %>%
	full_join(select(vine3_nodes, label, x_pos, y_pos), by = c("from" = "label")) %>%
	rename(xstart = x_pos, ystart = y_pos) %>%
	full_join(vine3_nodes, by = c("to" = "label")) %>%
	rename(xend = x_pos, yend = y_pos) 

vine3_fruit_data %<>%
	left_join(., select(vine3_links, to_shoot_id, cane_id, xend, yend, quadrant), by = c("shoot_id" = "to_shoot_id")) 
