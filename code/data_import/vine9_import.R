#### data import ####
vine9_data <- read_csv("input/architecture/kiwimac_data_vine9.csv") %>%
...
vine9_fruit_data <- read_csv("input/fruit_data/fruit_data_vine9.csv") %>%
	mutate(shoot_id = paste(Vine, ShootID, sep = "-"))

#### data gathering ####

vine9_sources <- vine9_data %>%
	distinct(from) %>%
	rename(label = from)

vine9_targets <- vine9_data %>%
	distinct(to) %>%
	rename(label = to)

vine9_nodes <- full_join(vine9_sources, vine9_targets, by = "label") %>%
	arrange(label) %>%
	full_join(select(vine9_data, to, length, to_shoot_id, x, y, quadrant, diameter, cane_id, to_origin_id, base_origin_id, notes), by = c("label" = "to"))
...
vine9_links <- vine9_data %>%
	select(from, to) %>%
	full_join(select(vine9_nodes, label, x_pos, y_pos), by = c("from" = "label")) %>%
	rename(xstart = x_pos, ystart = y_pos) %>%
	full_join(vine9_nodes, by = c("to" = "label")) %>%
	rename(xend = x_pos, yend = y_pos) 

vine9_fruit_data %<>%
	left_join(., select(vine9_links, to_shoot_id, cane_id, xend, yend, quadrant), by = c("shoot_id" = "to_shoot_id")) 
