...
if(!file.exists(
	...
	all_shoot_data <- read_csv(here("workspace/all_shoot_data.csv"), col_types = "dcdddccccdc")
}
	...
	all_arch_data %<>%
		mutate(VineTreatmentOrder = factor(VineUUID, levels = c(1, 2, 3, 4, 6, 5, 9, 8, 7)),
			   VineTreatment = if_else(VineUUID %in% c(1, 4, 9), "1 Conv", 
			   					   if_else(VineUUID %in% c(2, 6, 8), "2 Stru",
			   					   	   if_else(VineUUID %in% c(3, 5, 7), "3 Spur", NA))),
			   VineRow = if_else(VineUUID %in% c(1:3), 1,
			   				 if_else(VineUUID %in% c(4:6), 2,
			   				 	   if_else(VineUUID %in% c(7:9), 3, NA)))) 
