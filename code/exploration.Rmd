...
# Replacing FruitPerShoot with a tallied value, as there were some innacuracies in the all_shoot_data values.	
		
#fruitdif <- all_fruit_data %>%	
#			  group_by(ShootUUID) %>% 
 # 				mutate(FruitPerShoot = n()) %>%
#				filter(FruitPerShoot < FruitPositionUpShoot) %>%
#				select(VineUUID, FruitUUID, ShootUUID, FruitPerShoot, FruitPositionUpShoot)

		write_csv(all_fruit_data, here("workspace/all_fruit_data.csv"), na = "")
```

```{r graphing_setup, echo = FALSE}
...
```{r kris_plots, echo = FALSE}

source(here("code/visualisation/kris_plots.R"))
