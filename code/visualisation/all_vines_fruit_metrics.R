...
ggplot(filter(all_arch_data, !is.na(SegmentEndX) & !is.na(SegmentEndY))) +
...
ggplot(filter(all_arch_data, !is.na(SegmentEndX) & !is.na(SegmentEndY)) ) +
	...
	geom_text(aes(2200, -2300, label = paste("crop:", cropload)),
			  ...
			  data = vine_order) +
	guides(size = FALSE) +
	ggtitle("Dry Matter by VineUUID & treatment") +
	theme_bw() +
	theme(plot.title = element_text(size = 22, hjust = 0.5),
		  axis.ticks = element_blank(),
		  axis.text = element_blank(),
...
#### Sources for individual vine fruit metric plots ####

for(vine_id in 1:9)
	source(paste0("code/visualisation/fruit_metrics/vine", vine_id, "_fruit_metrics.R"))
