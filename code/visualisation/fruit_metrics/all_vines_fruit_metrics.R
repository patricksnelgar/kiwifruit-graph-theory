	...
	all_arch_data %>%
		filter(!is.na(SegmentStartX) & !is.na(SegmentStartY) & !is.na(SegmentEndX) & !is.na(SegmentEndY)) %>%
			ggplot() +
				...
				geom_jitter(aes(SegmentEndY, SegmentEndX, colour = DryMatter),
							data = filter(all_fruit_data, !is.na(dm_bins) & DryMatter > 14),
							alpha = 0.8,
							size = 7,
							width = 10,
							height = 70,
							shape = 19) +
				scale_colour_viridis(option = "D", direction=1, na.value = NA, name="Dry Matter (%)") +
				labs(x = NULL, y = NULL) +
	...
	ggsave(here("output/Vine 1 - 9 dry matter by treatment map.png"), width = 20, height = 15)
	...
	#### Flower Timing by treatment ####
		...
		ggplot() +
			geom_segment(aes(x = SegmentStartY, y = SegmentStartX, xend =  SegmentEndY, yend =  SegmentEndX, size = SegmentDiameter), colour = "lightgrey") +
			scale_size_continuous(name = "Segment diameter", breaks = pretty_breaks(10)) +
			geom_jitter(aes(SegmentEndY, SegmentEndX, colour = factor(FloweringDate)),
						...
						height = 70,
						shape =	19) +
			scale_colour_manual(values= c("#800101", "#785000", "#747800", "#077000", "#82ff8e", "#82d7ff", "#2400a8", "#7e00a8", "#a10d83")) +
	...
	ggsave(here("output/Vine 1 - 9 flowering date by treatment map.png"), width = 20, height = 15)
	...
	all_arch_data %>%
		...
		geom_segment(aes(x = SegmentStartY, y = SegmentStartX, xend =  SegmentEndY, yend =  SegmentEndX, size = SegmentDiameter), colour = "lightgrey") +
		scale_size_continuous(name = "Segment diameter", breaks = pretty_breaks(10)) +
		geom_jitter(aes(SegmentEndY, SegmentEndX, colour = AverageHueAngle),
					data = all_fruit_data,
					alpha = 1,
					size = 5,
					width = 10,
					...
					shape = 19, 
					na.rm=TRUE) +
		scale_colour_viridis(option = "D", direction=-1, na.value = NA, name="Hue Angle (°)") +
		labs(x = NULL, y = NULL) +
		facet_grid(vars(VineRow), vars(VineTreatment), labeller = labeller(VineTreatment = column_labels)) +
		geom_text(aes(-2450, -2300, label = vine_label), 
				  size = 7,
				  data = vine_order) + 
		guides(size = FALSE) +
		ggtitle("Flesh Hue Angle by Vine & treatment") +
		theme_base_graph()
	
	ggsave(here("output/Vine 1 - 9 Hue angle by treatment map.png"), width = 20, height = 15)
	...
	all_arch_data %>%
		...
		geom_segment(aes(x = SegmentStartY, y = SegmentStartX, xend =  SegmentEndY, yend =  SegmentEndX, size = SegmentDiameter), colour = "lightgrey") +
		scale_size_continuous(name = "Segment diameter", breaks = pretty_breaks(10)) +
		geom_jitter(aes(SegmentEndY, SegmentEndX, colour = SolubleSolidsContent),
					data = all_fruit_data,
					alpha = 1,
					size = 5,
					width = 10,
					...
					shape = 19, 
					na.rm=TRUE) +
		scale_colour_viridis(option = "D", direction=1, na.value = NA, name="Soluble Solids \n Content (°Brix)") +
		labs(x = NULL, y = NULL) +
		facet_grid(vars(VineRow), vars(VineTreatment), labeller = labeller(VineTreatment = column_labels)) +
		geom_text(aes(-2450, -2300, label = vine_label), 
				  size = 7,
				  data = vine_order) + 
		guides(size = FALSE) +
		ggtitle("Soluble Solids Content by Vine & treatment") +
		theme_base_graph()
	
	ggsave(here("output/Vine 1 - 9 Soluble Solids Content by treatment map.png"), width = 20, height = 15)
	...
	all_arch_data %>%
		...
		geom_segment(aes(x = SegmentStartY, y = SegmentStartX, xend =  SegmentEndY, yend =  SegmentEndX, size = SegmentDiameter), colour = "lightgrey") +
		scale_size_continuous(name = "Segment diameter", breaks = pretty_breaks(10)) +
		geom_jitter(aes(SegmentEndY, SegmentEndX, colour = AverageFirmness),
					data = all_fruit_data,
					alpha = 1,
					size = 5,
					width = 10,
					...
					shape = 19, 
					na.rm=TRUE) +
		scale_colour_viridis(option = "D", direction=-1, na.value = NA, name="Firmness (kgf)") +
		labs(x = NULL, y = NULL) +
		facet_grid(vars(VineRow), vars(VineTreatment), labeller = labeller(VineTreatment = column_labels)) +
		geom_text(aes(-2450, -2300, label = vine_label), 
				  size = 7,
				  data = vine_order) + 
		guides(size = FALSE) +
		ggtitle("Firmness by Vine & treatment") +
		theme_base_graph()
	
	ggsave(here("output/Vine 1 - 9 Firmness by treatment map.png"), width = 20, height = 15)
