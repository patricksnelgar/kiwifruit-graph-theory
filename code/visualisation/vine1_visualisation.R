...
vine1_basic_DryMatter <- ggplot(filter(vine1_links, !is.na(to))) +
	...
	geom_point(aes(x = yend, y = xend, colour = target_type), 
			   data = filter(vine1_links, !is.na(to) & target_type == "Shoot"),
			   shape = 19, 
			   size = 4) + 
	...
	geom_jitter(aes(yend, xend, fill = dm_bins), 
				data = filter(vine1_fruit_data, !is.na(dm_bins)), 
				alpha = 0.8, 
				size = 4,
				width = 10,
				height = 70,
	...
	guides(size = FALSE) +
	ggtitle("Kiwimac vine1 - basic Dry Matter heatmap") +
	labs(x = NULL, y = NULL) +
	theme_bw() + 
...
ggsave("./output/vine 1/basic Dry Matter heatmap.jpg", width = 16, height = 12)
